...
def create_tables_and_admin():
    with app.app_context():
        ...
        if not User.query.filter_by(faculty_id='admin').first():
            ...
            db.session.commit()
            log.info("Admin user created: admin / admin (change password ASAP)")
...
def release_slot():
    ...
    if not booking: return jsonify({'success': False, 'message': 'No booking found for that slot/date'}), 404
...
def block_slot():
    ...
    if not slot: return jsonify({'success': False, 'message': 'Slot not found'}), 404
    if new_status not in ['Free', 'Regular']: return jsonify({'success': False, 'message': 'Invalid status'}), 400
...
def create_user():
    ...
    if not name or not faculty_id or not password: return jsonify({'success': False, 'message': 'name, faculty_id and password required'}), 400
    if User.query.filter_by(faculty_id=faculty_id).first(): return jsonify({'success': False, 'message': 'faculty_id already exists'}), 400
...
def delete_user(user_id):
    ...
    if req_user and req_user.faculty_id == u.faculty_id: return jsonify({'success': False, 'message': 'Admin cannot delete themselves'}), 400
    if u.is_admin:
        admins_count = User.query.filter_by(is_admin=True).count()
        if admins_count <= 1: return jsonify({'success': False, 'message': 'Cannot delete the last admin'}), 400
    db.session.delete(u)
    db.session.commit()
    return jsonify({'success': True})
...
def set_weekend_default():
    ...
    ok, resp = is_requester_admin_from_args_or_json()
    if not ok: return jsonify({'success': False, 'message': resp}), 403
    lab_id = data.get('lab_id'); day = data.get('day'); custom_text = data.get('custom_text')
    if day not in ['Saturday','Sunday']: return jsonify({'success': False, 'message': 'Invalid day'}), 400
    if lab_id == 'global' or lab_id is None:
        ...
        if not Lab.query.get(lab_val): return jsonify({'success': False, 'message': 'Lab not found'}), 404
...
def set_weekend_override():
    ...
    if not ok: return jsonify({'success': False, 'message': resp}), 403
    admin_user = resp
    lab_id = data.get('lab_id'); day = data.get('day'); source_day = data.get('source_day')
    if day not in ['Saturday','Sunday']: return jsonify({'success': False, 'message': 'Invalid day'}), 400
    try:
        lab_val = int(lab_id)
    except Exception:
    ...
    if not ov:
        ov = WeekendOverride(lab_id=lab_val, day=day, target_date=target_date, override_type='follow', source_day=source_day, created_by=admin_user.id if admin_user else None)
        db.session.add(ov)
    else:
